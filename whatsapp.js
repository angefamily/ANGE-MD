...
// Détruit proprement le client courant (ferme Chrome) et réinitialise l'état,
// pour ne jamais rester bloqué après un échec/timeout/déconnexion.
async function resetClient() {
    ...
    if (stale) {
        try {
            ...
            // rien à faire si Chrome est déjà mort
        }
    }
}
...
function buildSock(clientInstance) {
    return {
        ...
        async sendMessage(jid, content = {}, options = {}) {
            ...
            // Comportement plus humain : indicateur "en train d'écrire..." +
            // délai proportionnel à la longueur du texte (avec un peu d'aléatoire),
            // pour éviter un pattern de réponse instantanée trop robotique.
            if (content.text && !content.react) {
                try {
                    ...
                    // si le typing indicator échoue, on envoie quand même le message
                }
            }
...
function wireEvents(clientInstance, sock) {
    clientInstance.on('message_create', async (msg) => {
        try {
            ...
            // On ignore les messages sortants normaux (envoyés par le bot ou depuis
            // le téléphone à quelqu'un d'autre) — sauf le cas particulier de la
            // conversation "avec soi-même", où fromMe est toujours vrai.
...
module.exports = { connectToWhatsApp, getStatus, resumeIfSessionExists };